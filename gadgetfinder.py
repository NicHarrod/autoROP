...
def search_gadget(gadget_list, target_start, requires_ret=True):
    ...
    for addr, instruction in gadget_list:
        ...
        if instruction.startswith(target_start):
            ...
            # assume the first part is our target. 
            # Every subsequent 'pop' before 'ret' is a value to pad.
            junk_count = 0
            for part in parts[1:]:
                if part == 'ret': 
                    break
...
def extract_gadgets():
    ...
    #  Parse ropchain.txt to extract gadgets
    parsed_gadgets = []
    ...
    #Retrieve Gadgets using dynamic search
    try:
...
if __name__ == "__main__":
    find_gadgets("./binaries/vuln3-32")
